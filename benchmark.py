...
variable_range = (-600, 600)  # Range for each variable

# Function to initialize a population with random values
...
# Function to evaluate the Griewank Function (De Jong Function 5)
def evaluate_griewank(solution):
    n = len(solution)
    sum_term = np.sum([x**2 for x in solution]) / 4000.0
    prod_term = np.prod([np.cos(x / np.sqrt(i + 1)) for i, x in enumerate(solution)])
...
for generation in range(num_generations):
    ...
    fitness_values = [evaluate_griewank(individual) for individual in population]
...
best_fitness = evaluate_griewank(best_solution)
